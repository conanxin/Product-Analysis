...
## 8. GitHub Actions CI (P25)

P25 把本地验证脚本接入 GitHub Actions CI，提供自动拦截机制。

**CI 文件：** `.github/workflows/ai-analysis-index-check.yml`

**触发时机：**
- `push` — 每次 push 到 master 时自动运行
- `pull_request` — 每次 PR 创建/更新时自动运行
- `workflow_dispatch` — 手动从 GitHub Actions 页面触发

**CI 做什么：**
1. checkout 仓库
2. setup Python 3.x
3. 安装 PyYAML
4. 运行 `python3 scripts/verify_ai_analysis_index.py`

**失败时如何处理：**
1. 查看 workflow log (GitHub Actions 页面 → 失败的 run)
2. 本地运行同一命令: `python3 scripts/verify_ai_analysis_index.py`
3. 根据错误列表修复:
   - `README.md` 质量状态表数字
   - `analyses/README.md` 质量状态表数字
