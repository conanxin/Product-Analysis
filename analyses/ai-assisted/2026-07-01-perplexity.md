...
source_urls:
  ...
  # High-quality media - INFERRED URLs (NOT verified by HTTP fetch; use only with secondary confirmation)
...
source_quality_notes: P4.1 实链验证:仅 AWS 官方案例直接 verified;perplexity.ai/.enterprise 超时但仍推定可达;6 个高质量媒体 URL (CNBC×3/Verge/MacRumors/Information/Reuters) 在 P4.1 实链验证中未能验证 (404/403/timeout), 仅能依靠中文转载佐证;SourceForge 被 Cloudflare 拦截;中文转载为唯一被验证可访问的报道载体,需进一步补全原文 URL。
...
| 2025-08 估值 $20B、2025-09 The Information 报道 $2B 资金 | **中→低** | The Information 需订阅(Cloudflare 拦截),P4.1 未验证原文 | **P4.1 降为低**:未能实访原文,仅依靠中文转载;P4.1 后该事实需进一步核验 |
| 拟以 $345 亿收购 Chrome(2025-08) | **中→低** | CNBC URL 404,WSJ 未实访 | **P4.1 降为低**:CNBC 路径未能验证,仅依靠中文转载 |
| Microsoft 与 Perplexity $7.5 亿 Azure 协议(2026-01) | **中→低** | CNBC URL 404 | **P4.1 降为低**:CNBC 路径未能验证,仅依靠中文转载 |
...
本节记录人工实链验证结果。验证方式:使用 `web_fetch` 对 Sources 中 10 个主体质量来源做 HTTP 访问。
...
- **仅 1 个主体来源被明确验证**:AWS 官方案例。
- **7 个 high-quality-media-verified (planned) URL 中 6 个未能验证**:4 个 404/400/timeout,1 个 522,1 个需订阅被 Cloudflare 拦截。
- **Verifier 判定**:主要价值事件(Comet 免费、PayPal 合作、Microsoft Azure 协议、NYT 起诉、估值、Mac 桌面应用)在 P4.1 阶段仍需依靠中文转载佐证。**P4 中 Source Quality 以"高质量媒体为主"的声明需修正。**
- **诚实评估**:本轮"P4 以 CNBC/Verge/Reuters/MacRumors 作为主来源"的描述是高估的。**实际上主要事实依据还是中文转载。**

**需后续补全**:

- P5 阶段:手动查实 CNBC、The Verge、Reuters、MacRumors 的实际原报道 URL;补全后重设 verification_status 为 verified。
- 在未补全前,本文章 `review_status` 保持 `reviewed`,但"已人工复核"的语义需限定为"事实框架已核验,具体事实仍需原文核实"。
- 参考 `docs/source-quality-checklist.md`(P4.1 新增)作为后续的质量门禁。
...
### Quality Media / Direct Reporting(P4.1 实链验证:未验证)

**重要说明:P4 中加入的 7 个高质量媒体 URL 在 P4.1 阶段未能通过 HTTP 实链验证。** 表现为:CNBC × 3 为 404、Verge 路径返回 400、MacRumors 522、Reuters timeout、The Information 被 Cloudflare 拦截。这些 URL 可能是路径推定错误、需登录订阅或被防抓取。**事实依据主要依靠下方"中文转载"组的转载转引。**

| URL | 来源类型 | 状态 | 用途 | 备注 |
|-----|----------|------|------|------|
| https://www.cnbc.com/2025/10/02/perplexity-comet-ai-browser-free-worldwide.html | CNBC | unverified (404) | Comet 全球免费发布(2025-10) | 推测路径,未能实访;需查实实际 CN报发表 URL |
| https://www.cnbc.com/2025/09/03/paypal-venmo-perplexity-comet-ai-browser.html | CNBC | unverified (404) | PayPal/Venmo 12 个月 Max 订阅(2025-09) | 推测路径,未能实访 |
| https://www.cnbc.com/2026/01/30/perplexity-microsoft-azure-750-million-deal.html | CNBC | unverified (404) | Microsoft Azure 7.5 亿协议(2026-01) | 推测路径,未能实访 |
| https://www.macrumors.com/2026/05/08/perplexity-mac-personal-computer.html | MacRumors | unverified (522) | Personal Computer Mac 应用(2026-05) | 服务器超时或路径问题 |
| https://www.theverge.com/2025/7/9/perplexity-comet-ai-browser.html | The Verge | unverified (400) | Comet 推出 + Aravind Srinivas 采访 | 路径格式错误 |
| https://www.theinformation.com/articles/perplexity-200-billion-valuation | The Information | paywalled | 200 亿估值 / $2B 资金报道 | 需订阅,被 Cloudflare 拦截 |
| https://www.reuters.com/technology/perplexity-new-york-times-lawsuit-2025-12-06/ | Reuters | unverified (timeout) | 纽约时报起诉(2025-12) | 路径待查实 |
...
*状态：reviewed（事实框架已核验）；source_url_verification_status: partial（具体事实验证不完整）*
